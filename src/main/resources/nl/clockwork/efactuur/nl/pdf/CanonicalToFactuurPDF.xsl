...
<!--
	...
	Author: Edwin Luinstra, Rienk Prinssen, Bas van Driel 
-->
<xsl:stylesheet xpath-default-namespace="http://www.clockwork.nl/ezp/pdf/canonical" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:fox="http://xmlgraphics.apache.org/fop/extensions" xmlns:nl="http://ns.hr-xml.org/2007-04-15" version="2.0">
	...
	<xsl:template name="factuur_info">
		<fo:block>
			Debiteurennummer: <xsl:value-of select="/bericht/debiteur/registratienummer"/>
		</fo:block>
		<fo:block>
			Factuurnummer: <xsl:value-of select="/bericht/factuur/factuurnummer"/>
		</fo:block>
		<fo:block>
			Factuurdatum: <!--xsl:value-of select="format-dateTime(/bericht/factuur/datum,$date_format)"/--><xsl:apply-templates select="/bericht/factuur/datum" mode="date"/>
		</fo:block>
		<fo:block>
			<xsl:call-template name="vervaldatum"></xsl:call-template>			
		</fo:block>
		<fo:block>
			Inkooporder: <xsl:value-of select="/bericht/factuur/inkooporder"/>
		</fo:block>
	</xsl:template>
...
<!-- 		<fo:block> -->
<!-- 			Registratienummer: <xsl:value-of select="/bericht/crediteur/registratienummer"/> -->
<!-- 		</fo:block> -->
		<fo:block>
			IBAN: <xsl:value-of select="/bericht/crediteur/bankrekening/nummer"/>
		</fo:block>
		<fo:block>
			BIC: <xsl:value-of select="/bericht/crediteur/bankrekening/bic"/>
		</fo:block>
		<fo:block>
			KvK: <xsl:value-of select="/bericht/crediteur/kvk_nummer"/>
		</fo:block>
	</xsl:template>
	...
	<xsl:template name="referentie">
		<xsl:choose>
			<xsl:when test="/bericht/factuur/referentie/contract/nummer != '' or /bericht/factuur/referentie/factuur_oorspronkelijk/factuurnummer_leverancier != '' or /bericht/factuur/referentie/factuur_oorspronkelijk/datum != ''">
				...
				<xsl:if test="/bericht/factuur/referentie/contract/nummer != ''">
					<fo:block>
						Contractnummer: <xsl:value-of select="/bericht/factuur/referentie/contract/nummer"/>
					</fo:block>
				</xsl:if>
	
				<xsl:if test="/bericht/factuur/referentie/factuur_oorspronkelijk/factuurnummer_leverancier != ''">
					<fo:block>
						Factuurnummer: <xsl:value-of select="/bericht/factuur/referentie/factuur_oorspronkelijk/factuurnummer_leverancier"/>
					</fo:block>
				</xsl:if>
				
				<xsl:if test="/bericht/factuur/referentie/factuur_oorspronkelijk/datum != ''">
					<fo:block>
						Factuurdatum: <xsl:value-of select="/bericht/factuur/referentie/factuur_oorspronkelijk/datum"/>
					</fo:block>
				</xsl:if>
	...
	<!--xsl:template match="*" mode="betalingsconditie">
		<fo:block>
			<xsl:value-of select="local-name()"/>: <xsl:value-of select="child::node()"/>
		</fo:block>
	</xsl:template-->

	<xsl:template match="omschrijving" mode="betalingsconditie">
		<fo:block>
			Omschrijving: <xsl:value-of select="child::node()"/>
		</fo:block>
	</xsl:template>

	<xsl:template match="kortingspercentage" mode="betalingsconditie">
		<fo:block>
			Kortingspercentage: <xsl:apply-templates select="child::node()" mode="number"/>%
		</fo:block>
	</xsl:template>

	<xsl:template match="setu_aantal_dagen" mode="betalingsconditie">
		<fo:block>
			Aantal Dagen: <xsl:apply-templates select="child::node()" mode="number"/>
		</fo:block>
	</xsl:template>

	<xsl:template match="setu_start_datum" mode="betalingsconditie">
		<fo:block>
			Startdatum: <!--xsl:value-of select="format-dateTime(child::node(),$date_format)"/--><xsl:apply-templates select="child::node()" mode="date"/>
		</fo:block>
	</xsl:template>

	<xsl:template match="setu_vervaldatum" mode="betalingsconditie">
		<fo:block>
			Vervaldatum: <!--xsl:value-of select="format-date(child::node(),$date_format)"/--><xsl:apply-templates select="child::node()" mode="date"/>
		</fo:block>
	</xsl:template>

	<xsl:template match="ubl_looptijd" mode="betalingsconditie">
		<fo:block>
			Looptijd: <xsl:value-of select="child::node()"/> <xsl:value-of select="@type"/>
		</fo:block>
	</xsl:template>
		...
		<fo:block>
			<xsl:for-each select="/bericht/bijlages/bijlage">
				<fo:block id="{@id}" break-before="page" font-weight="bold">
					Bijlage <xsl:value-of select="@id"/>
				</fo:block>
				...
				<fo:table-column column-width="25%"/>
	 			...
	 			<fo:table-body>
	 				...
	 				<fo:table-row>
	 					...
	 					<fo:table-cell>
	 						<fo:block><xsl:value-of select="nl:ReportedResource/nl:Person/nl:Id/nl:IdValue"/></fo:block>
	 					</fo:table-cell>
	 				</fo:table-row>
	 				<fo:table-row>
	 					<fo:table-cell>
	 					...
	 					<fo:table-cell>
	 						<fo:block><xsl:value-of select="nl:ReportedResource/nl:Person/nl:PersonName/nl:LegalName"/></fo:block>
	 					</fo:table-cell>
	 				</fo:table-row>
	 				...
	 				<fo:table-row>
	 					...
	 					<fo:table-cell>
	 						<fo:block><xsl:apply-templates select="nl:StartDateTime" mode="date"/></fo:block>
	 					</fo:table-cell>
	 				</fo:table-row>
	 				<fo:table-row>
	 					<fo:table-cell>
	 					...
	 					<fo:table-cell>
	 						<fo:block><xsl:apply-templates select="nl:EndDateTime" mode="date"/></fo:block>
	 					</fo:table-cell>
	 				</fo:table-row>
	 				<fo:table-row>
	 					<fo:table-cell>
	 				...
	 				<fo:table-row>
	 					...
	 					<fo:table-cell>
	 						<fo:block><xsl:apply-templates select="nl:Duration" mode="number"/></fo:block>
	 					</fo:table-cell>
	 				</fo:table-row>
	 				<fo:table-row>
	 					<fo:table-cell>
	 					...
	 					<fo:table-cell>
	 						<fo:block><xsl:apply-templates select="nl:Comment"/></fo:block>
	 					</fo:table-cell>
	 				</fo:table-row>
					</xsl:for-each>
	 				...
	 				<fo:table-row>
	 					<fo:table-cell number-columns-spanned="2">
	 						<fo:block font-weight="bold">
	 							Vergoeding <xsl:value-of select="@type"/>
	 						</fo:block>
	 					</fo:table-cell>
	 				</fo:table-row>
	 				<fo:table-row>
	 					...
	 					<fo:table-cell>
	 						<fo:block><xsl:apply-templates select="nl:StartDate" mode="date"/></fo:block>
	 					</fo:table-cell>
	 				</fo:table-row>
	 				<fo:table-row>
	 					<fo:table-cell>
	 					...
	 					<fo:table-cell>
	 						<fo:block><xsl:apply-templates select="nl:EndDate" mode="date"/></fo:block>
	 					</fo:table-cell>
	 				</fo:table-row>
	 				<fo:table-row>
	 					<fo:table-cell>
	 					...
	 					<fo:table-cell>
	 						<fo:block><xsl:apply-templates select="nl:Amount" mode="number"/><xsl:text> </xsl:text><xsl:value-of select="nl:Amount/@currency"/></fo:block>
	 					</fo:table-cell>
	 				</fo:table-row>
	 				<fo:table-row>
	 					<fo:table-cell>
	 					...
	 					<fo:table-cell>
	 						<fo:block><xsl:apply-templates select="nl:Quantity" mode="number"/></fo:block>
	 					</fo:table-cell>
	 				</fo:table-row>
					</xsl:for-each>
	 				<fo:table-row>
	 				...
	 				<fo:table-row>
	 					...
	 					<fo:table-cell>
	 						<fo:block><xsl:value-of select="nl:AdditionalData/nl:StaffingAdditionalData/nl:CustomerReportingRequirements/nl:PurchaseOrderNumber"/></fo:block>
	 					</fo:table-cell>
	 				</fo:table-row>
	 				<fo:table-row>
	 					<fo:table-cell>
	 					...
	 					<fo:table-cell>
	 						<fo:block><xsl:value-of select="nl:AdditionalData/nl:StaffingAdditionalData/nl:CustomerReportingRequirements/nl:ProjectCode"/></fo:block>
	 					</fo:table-cell>
	 				</fo:table-row>
	 				<fo:table-row>
	 					<fo:table-cell>
	...
	<xsl:template name="addresstemplate">
		...
		<xsl:if test="postbusnummer!=''">
			<fo:block>
				Postbus <xsl:value-of select="postbusnummer"/>
			</fo:block>
		</xsl:if>
		<xsl:if test="straat!=''">
			<fo:block>
				<!-- DIGIFACT-45 -->
				<xsl:value-of select="straat"/><xsl:text> </xsl:text><xsl:value-of select="huisnummer"/>
			</fo:block>
		</xsl:if>
