...
<!--PATTERN cardinality-redefines-->
	...
	<!--RULE -->
<xsl:template match="doc:ApplicationResponse" priority="1005" mode="M4">

		<!--ASSERT -->
<xsl:choose>
         <xsl:when test="count(cbc:CustomizationID) &gt;= 1"/>
         <xsl:otherwise>doc:ApplicationResponse must contain cbc:CustomizationID at least 1 time(s)<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
...
<xsl:choose>
         ...
         <xsl:otherwise>doc:ApplicationResponse must contain cbc:ProfileID at least 1 time(s)<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
...
<xsl:choose>
         ...
         <xsl:otherwise>doc:ApplicationResponse must contain cbc:UBLVersionID at least 1 time(s)<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
...
<xsl:choose>
         ...
         <xsl:otherwise>doc:ApplicationResponse/cac:DocumentResponse/cac:Response may contain cbc:Description at most 1 time(s)<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*|comment()|processing-instruction()" mode="M4"/>
   </xsl:template>
...
<xsl:choose>
         ...
         <xsl:otherwise>doc:ApplicationResponse/cac:ReceiverParty must contain cac:PartyIdentification at least 1 and at most 2 time(s)<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
...
<xsl:choose>
         ...
         <xsl:otherwise>doc:ApplicationResponse/cac:ReceiverParty may contain cac:PartyName at most 1 time(s)<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*|comment()|processing-instruction()" mode="M4"/>
   </xsl:template>
...
<xsl:choose>
         ...
         <xsl:otherwise>doc:ApplicationResponse/cac:ReceiverParty/cac:PartyIdentification/cbc:ID must contain @schemeAgencyName at least 1 time(s)<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*|comment()|processing-instruction()" mode="M4"/>
   </xsl:template>
...
<xsl:choose>
         <xsl:when test="count(cac:PartyIdentification) &gt;= 1 and count(cac:PartyIdentification) &lt;= 2"/>
         <xsl:otherwise>doc:ApplicationResponse/cac:SenderParty must contain cac:PartyIdentification at least 1 and at most 2 time(s)<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*|comment()|processing-instruction()" mode="M4"/>
   </xsl:template>
...
<xsl:choose>
         <xsl:when test="count(@schemeAgencyName) &gt;= 1"/>
         <xsl:otherwise>doc:ApplicationResponse/cac:SenderParty/cac:PartyIdentification/cbc:ID must contain @schemeAgencyName at least 1 time(s)<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*|comment()|processing-instruction()" mode="M4"/>
   </xsl:template>
   ...
   <!--PATTERN type-restrictions-->
	...
	<!--RULE -->
<xsl:template match="doc:ApplicationResponse" priority="1004" mode="M5">
...
<xsl:choose>
         ...
         <xsl:otherwise>doc:ApplicationResponse/cac:DocumentResponse/cac:DocumentReference/cbc:DocumentType must have one of the following values: [inkooporder, bestellingsaanvraag]<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*|comment()|processing-instruction()" mode="M5"/>
   </xsl:template>
...
<xsl:choose>
         ...
         <xsl:otherwise>Element lengte MOET minder dan 11 posities zijn.<xsl:value-of select="string('&#xA;')"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*|comment()|processing-instruction()" mode="M5"/>
   </xsl:template>
